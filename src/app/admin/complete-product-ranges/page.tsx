...
interface CompleteProductRange {
  _id: string;
  title: string;
  slug: string;
  description: string;
...
export default function CompleteProductRangeManagement() {
  const [ranges, setRanges] = useState<CompleteProductRange[]>([]);
  const [loading, setLoading] = useState(true);
  const [search, setSearch] = useState('');
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [editingRange, setEditingRange] = useState<CompleteProductRange | null>(null);
  ...
  async function fetchRanges() {
    setLoading(true);
    try {
      const res = await fetch('/api/complete-product-ranges?admin=true');
      if (res.ok) {
        const data = await res.json();
        setRanges(Array.isArray(data) ? data : []);
      }
    } catch (err) {
      toast.error('Failed to load complete product ranges');
    } finally {
      setLoading(false);
    }
  }

  const handleOpenModal = (range: CompleteProductRange | null = null) => {
    if (range) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const url = editingRange ? `/api/complete-product-ranges/${editingRange._id}` : '/api/complete-product-ranges';
    ...
    try {
      ...
      if (res.ok) {
        toast.success(editingRange ? 'Complete Product Range updated' : 'Complete Product Range created');
        fetchRanges();
        handleCloseModal();
      } else {
  ...
  async function deleteRange(id: string) {
    if (!confirm('Delete this complete product range? This cannot be undone.')) return;
    try {
      const res = await fetch(`/api/complete-product-ranges/${id}`, { method: 'DELETE' });
      if (res.ok) {
        toast.success('Complete Product Range removed');
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500 font-medium text-sm italic">Manage the high-level complete product series shown on your landing page.</p>
        </div>
        <button 
      ...
      <div className="bg-white border border-gray-100 shadow-sm">
        <div className="p-4 border-b border-gray-100 flex items-center bg-gray-50/50">
          ...
          <input 
            type="text" 
            placeholder="Search complete product ranges..." 
        ...
        <div className="p-6">
          {loading ? (
            ...
            <div className="py-12 text-center text-gray-400 text-xs font-black uppercase tracking-widest">No complete product ranges found</div>
          ) : (
          ...
          <div className="bg-white w-full max-w-3xl border border-gray-100 shadow-2xl animate-in zoom-in-95 duration-200 my-auto">
            <div className="p-6 border-b border-gray-100 flex justify-between items-center bg-secondary text-white">
              <h2 className="text-xl font-black uppercase tracking-tighter flex items-center text-white">
                ...
                {editingRange ? 'Edit Complete Range' : 'Add New Complete Range'}
              </h2>
